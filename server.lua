...
-- ✅ seznam právníků, kteří jsou "eligible" (mají lawyer job a 0★)
local EligibleLawyers = {}
...
-- ===== ✅ Lawyer list sync (NO kg_job) =====
local function recomputeLawyerEligibility(src)
    local x = ESX.GetPlayerFromId(src)
    if not x then
        EligibleLawyers[src] = nil
        return false
    end

    local stars = tonumber(Player(src).state.kg_wanted or 0) or 0
    local ok = isLawyer(x) and stars <= 0

    local before = EligibleLawyers[src] == true
    if ok then EligibleLawyers[src] = true else EligibleLawyers[src] = nil end
    local after = EligibleLawyers[src] == true

    return before ~= after
end

local function getEligibleLawyerList()
    local list = {}
    for sid, _ in pairs(EligibleLawyers) do
...
local function pushLawyersTo(src, list)
    local stars = tonumber(Player(src).state.kg_wanted or 0) or 0
    if stars > 0 then
        TriggerClientEvent('kg_wanted:updateLawyers', src, list or getEligibleLawyerList())
    else
        TriggerClientEvent('kg_wanted:updateLawyers', src, {})
    end
end

local function pushLawyersToAllWanted()
    local list = getEligibleLawyerList()
    for _, pid in ipairs(GetPlayers()) do
        local src = tonumber(pid)
        if src and GetPlayerName(src) then
            local stars = tonumber(Player(src).state.kg_wanted or 0) or 0
            if stars > 0 then
                TriggerClientEvent('kg_wanted:updateLawyers', src, list)
            end
        end
    end
end

-- Periodic keep-alive (kdyby se něco rozjelo)
CreateThread(function()
    while true do
        Wait(15000)
        pushLawyersToAllWanted()
    end
end)
...
local function setStars(src, stars, reason)
    ...
    -- ✅ pokud se změnila eligibility právníka, přepošleme list wanted hráčům
    local changed = recomputeLawyerEligibility(src)
    if changed then
        pushLawyersToAllWanted()
    else
        -- když se někomu změní wanted, chceme mu poslat list (aby hned viděl právníky)
        pushLawyersTo(src)
    end
...
RegisterNetEvent('kg_wanted:crime', function(data)
    ...
    local victimStars = Player(victim).state.kg_wanted or 0
    ...
    -- 🔴 POLICE CODEX LOGIKA
    if isCop and (tonumber(victimStars) or 0) <= 0 then
...
-- ✅ Job změna: přepočítat eligibility právníka + poslat list wanted hráčům
RegisterNetEvent('esx:setJob', function(_, _)
    local src = source
    local changed = recomputeLawyerEligibility(src)
    if changed then
        pushLawyersToAllWanted()
...
RegisterNetEvent('esx:playerLoaded', function(playerId)
    ...
    if not (Config.Persistence and Config.Persistence.Enabled) then
        ...
        recomputeLawyerEligibility(src)
        pushLawyersTo(src)
        pushLawyersToAllWanted()
        return
    end
